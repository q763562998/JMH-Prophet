...
class SeqEncoder(nn.Module):
    ...
    def forward(self, inputs, input_lens=None): 
        ...
        if input_lens is not None:
            ...
            inputs = pack_padded_sequence(inputs_sorted, input_lens_sorted.dat.a.tolist(), batch_first=True)
...
class MultiModel(torch.nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels, num_layers):
        ...
        self.fuse = nn.Linear(n_hidden*2,n_hidden)
    ...
    def code_encoding(self, name_repr, graphseq_repr):
        ...
        attn_cat = torch.cat([name_attn_scalar, graphseq_attn_scalar], 1)
        ...
        cat_atten_repr = torch.cat((name_feat_atten, graphseq_feat_atten), 2)
        code_repr = torch.tanh(self.fuse(F.dropout(cat_atten_repr, 0.25, training=self.training))).reshape(-1,self.hidden)
        return code_repr 

    def forward(self, tree_x,tree_edge_index,tree_batch,graph_x,graph_edge_index,graph_batch):
        ...
        x=self.code_encoding(tok_repr,graphseq_repr)
